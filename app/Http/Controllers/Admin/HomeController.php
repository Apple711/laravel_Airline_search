...
class HomeController extends Controller
{
    ...
    public function search(Request $request){
        ...
        if ( $customer_type == "mro" ){
            $results = Mrocompany::join('contacts','contacts.companyid','mrocompanies.id')->where('applist', 'like', '%,'.$application_id.',%')->get();
            ...
            $results = Airline::join('contacts','contacts.airline','airlines.operator')->select('airlines.operator','airlines.aircraftSeries','airlines.engineType','airlines.engineModel','airlines.apuModel','contacts.email','contacts.title')->where(function($query) use ($applist){
            ...
            $airline_results = Airline::join('contacts','contacts.airline','airlines.operator')->select('airlines.operator','airlines.aircraftSeries','airlines.engineType','airlines.engineModel','airlines.apuModel','contacts.email','contacts.title')->where(function($query) use ($applist){
                $query->where('airlines.aircraftSeries', 'LIKE', '%'.$applist.'%');
                $query->orWhere('airlines.engineType', 'LIKE', '%'.$applist.'%');
                $query->orWhere('airlines.engineModel', 'LIKE', '%'.$applist.'%');
                $query->orWhere('airlines.apuModel', 'LIKE', '%'.$applist.'%');
