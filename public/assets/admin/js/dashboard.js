    ...
    $.get(
        ...
        function (result) {
            if (result.status == "ok") {
                ...
                var drivers=result.driver_detail_array;
                $(".period_total .state_table_emp tr").remove();

                var html = "";
                for (var i = 0; i < data.length; i++) {
                    html += "<tr class='employee-detail'>" +
                        "<td><a href='#'  onclick='showDetail(event,"+ data[i].id +")'>" + data[i].employee_name + "</a></td>" +
                        ...
                        "</tr>";
                    var emp_drivers = drivers[data[i].id];
                    for(var j = 0; j< emp_drivers.length; j++){
                        html += "<tr class='driver-detail driver-detail-" + data[i].id + "'>" +
                        "<td class='driver-name'>" + emp_drivers[j].driver_name + "</td>" +
                        "<td>" + emp_drivers[j].total_miles + "</td>" +
                        "<td>" + emp_drivers[j].detail_num + "</td>" +
                        "<td>" + emp_drivers[j].total_revenue + "</td>" +
                        "<td>" + emp_drivers[j].avg_revenue + "</td>" +
                        "<td>" + emp_drivers[j].avg_dhrpm + "</td>" +
                        "</tr>";
                    }
                }
...
function showDetail(event,emp_id) {
    event.preventDefault();
    $('.driver-detail-' + emp_id).toggle();
}